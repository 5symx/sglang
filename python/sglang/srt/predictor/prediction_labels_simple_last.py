# predict topk by using label of last layers. (hidden state to [0,0,1,0 etc])
...
def load_data(json_path):
    if os.path.exists(json_path):
        ...
        assert labels.shape[1] ==  num_experts
...
def evaluate(model, dataloader, criterion):
    ...
    with torch.no_grad():
        for emb, lbl in dataloader:
            ...
            # Top-k accuracy
            topk_pred = torch.topk(logits, k=k_pred, dim=-1).indices
            topk_true = torch.topk(lbl, k=k_true, dim=-1).indices

            
            # topk_true: (batch_size, k)

            # Compare predictions vs true experts
            match = (topk_pred.unsqueeze(2) == topk_true.unsqueeze(1))   # (batch_size, k, k)

            # Count how many true experts are matched
            num_matches = match.any(dim=1).sum(dim=1)   # (batch_size,) number of true experts found

            # Fraction of true experts recovered
            percent_overlap = num_matches.float() / k_true   # (batch_size,) percentage per sample

            # Accumulate average percentage
            correct_topk += percent_overlap.sum().item()

            # # topk_pred: (batch_size, k) for one of them lap with each other
...
def training(train_loader, test_loader, num_epochs = 5):
    ...
    # criterion = nn.KLDivLoss(reduction="batchmean") # expert count in size 128
    ...
    for epoch in range(num_epochs):
        ...
        for emb, lbl in train_loader:
            optimizer.zero_grad()
            logits = model(emb)
            # for nn.KLDivLoss(reduction="batchmean") 
            # lbl_probs = lbl / lbl.sum(dim=1, keepdim=True)
            # log_probs = F.log_softmax(logits, dim=1) # normalize predictions 
            # loss = criterion(log_probs, lbl_probs)

            # for criterion = nn.MSELoss()
            # loss = criterion(logits, lbl) 
            ...
            topk_pred  = torch.topk(logits, k=k_pred, dim=-1).indices  # (batch_size, 5)
            topk_true = torch.topk(lbl, k=k_true, dim=-1).indices
            ...
            # Count how many true experts are matched
            num_matches = match.any(dim=1).sum(dim=1)   # (batch_size,) number of true experts recovered
        ...
        print(f"Epoch {epoch+1}/{num_epochs}, " \
            f"Train Loss: {avg_loss:.4f}, Train Top-{k_pred} Acc: {acc_topk:.4f}, " \
            f"Test Loss: {test_loss:.4f}, Test Top-{k_pred} Acc: {test_acc:.4f}, ")
...
if __name__ == "__main__":
    ...
    json_path = "/home/ymx/sglang/scripts/my_test/output-122-mmlu.json"
    train_loader,test_loader = load_data(json_path=json_path)

    json_path_2 = "/home/ymx/sglang/scripts/my_test/output-122-gsm8k-1.json"
    train_loader_2,test_loader_2 = load_data(json_path=json_path_2)
    training(train_loader,test_loader, num_epochs=10)