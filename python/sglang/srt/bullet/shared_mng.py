...
from sglang.srt.managers.schedule_policy import CLIP_MAX_NEW_TOKENS # CLIP_MAX_NEW_TOKENS_ESTIMATION
...
class SharedManager:
    ...
    def __init__(self, args: ServerArgs, *, create=False):
        ...
        if hasattr(args, "predictor_param_file") and args.predictor_param_file:
            try:
                self.lib = ctypes.CDLL(args.predictor_param_file) # {BASE}/csrc/build/libsmctrl.so
    ...
    def set_adaptive_decode_num_tpcs(self, queue_ms) -> Tuple[int, SelectedSMPolicy]:
        ...
        if not self.lib:
            logger.warning(f"Shared Manager without lib return total_tpc : {self.shm_name}")
            return TOTAL_TPCS, SelectedSMPolicy.DEFAULT
